...
RUN apt-get update && \
    apt-get -y install autoconf automake build-essential libfreetype6-dev \
    libsdl1.2-dev libtheora-dev libtool libva-dev libvdpau-dev libxcb1-dev libxcb-shm0-dev \
...
RUN ZLIB="zlib-1.2.11" && vNGRTMP="v1.1.11" && PCRE="8.41" && nginx_build=/root/nginx && mkdir $nginx_build && \
    cd $nginx_build && \
    wget https://github.com/Meonardo/janus-webrtc-gateway-docker/raw/master/resources/pcre-8.41.tar.gz && \
...
RUN SRTP="2.4.2" && apt-get remove -y libsrtp0-dev && wget https://github.com/cisco/libsrtp/archive/v$SRTP.tar.gz && \
...
WORKDIR /tmp
RUN git clone https://github.com/Karlson2k/libmicrohttpd
...
RUN echo "Generate certificate files..."
COPY ./configs/localhost.conf /usr/local/localhost.conf
RUN sudo openssl req -x509 -nodes -days 730 -newkey rsa:2048 \
-keyout /etc/ssl/private/localhost.key \
-out /etc/ssl/certs/localhost.crt \
-config /usr/local/localhost.conf -sha256
